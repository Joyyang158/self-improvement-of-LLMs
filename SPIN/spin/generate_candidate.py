...
def parse_arguments():
    ...
    parser.add_argument('--input_dir', type=str, default='reformatted')
...
def main():
    ...
    with accelerator.split_between_processes(prompts_all) as prompts:
        ...
        for prompts_tokenized in tqdm(prompt_batches):
            ...
            try:
                outputs_tokenized=model.generate(**prompts_tokenized, max_new_tokens=512, pad_token_id=tokenizer.eos_token_id,
                do_sample = True, num_beams = 5, temperature = 1.0, top_k = 50, top_p = 0.9, num_return_sequences = args.num_return_sequences)
            except:
                continue
            

            # remove prompt from gen. tokens
            batch_size = len(prompts_tokenized['input_ids'])
            outputs_tokenized = outputs_tokenized.view(batch_size , args.num_return_sequences, -1)
            for i, tok_in in enumerate(prompts_tokenized['input_ids']):
                candidate_outputs = []
                for j in range(args.num_return_sequences):
                    tok_out = outputs_tokenized[i,j]
                    candidate_outputs.append(tok_out[len(tok_in):])
                candidate_texts = tokenizer.batch_decode(candidate_outputs)
                results.append(candidate_texts)
        ...
        results_cleaned = [[r.replace("</s>","").lstrip() for r in candidate_set] for candidate_set in results_gathered]
    ...
    if accelerator.is_local_main_process:
        ...
        for idx in range(len(corrects_all)):
            d = {"real": [{"role": "user", "content": prompts_old[idx]}, {"role": "assistant", "content": corrects_all[idx]}], "generated": [{"role": "user", "content": prompts_old[idx]}, {"role": "assistant", "content": results_cleaned[idx]}]}
            if args.split == 'test':
