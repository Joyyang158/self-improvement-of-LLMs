...
class SPINTrainer(Trainer):
    ...
    def __init__(
        ...
        self.model.fc_logvar = nn.Linear(4096, 32000)
    ...
    def spin_loss(
        ...
        #     if (policy_real_logps[i] - opponent_real_logps[i] < policy_generated_logps[i] - opponent_generated_logps[i]):
        ...
        ######## Add ########
        # real_difference = F.logsigmoid(policy_real_logps - opponent_real_logps)
        # losses = losses - real_difference
    ...
    def get_batch_metrics(
        ...
        metrics[f"{prefix}logps/opponent_real"] = opponent_real_logps.detach().cpu().mean()
        metrics[f"{prefix}logps/opponent_generated"] = opponent_generated_logps.detach().cpu().mean()
