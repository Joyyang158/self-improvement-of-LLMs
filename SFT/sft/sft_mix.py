...
def apply_chat_template(example, tokenizer):
    messages_real = example['real']
    messages_generated = example['generated']
    # if messages[0]["role"] != "system":
    #     messages.insert(0, {"role": "system", "content": ""})

    text_real = tokenizer.apply_chat_template(messages_real , tokenize=False)
    text_generated = tokenizer.apply_chat_template(messages_generated, tokenize=False)
    return {'text_real': text_real, 'text_generated': text_generated}
...
def combine_data(raw_dataset, split):
    df = raw_dataset[split].to_pandas()
    df_real = df[['text_real']].rename(columns = {'text_real':'text'})
    df_generated = df[['text_generated']].rename(columns = {'text_generated':'text'})
    df_combined = pd.concat([df_real, df_generated])
    final_dataset = Dataset.from_pandas(df_combined)
    shuffled_final_dataset = final_dataset.shuffle(seed = 42).remove_columns(['__index_level_0__'])
    return shuffled_final_dataset


raw_dataset = DatasetDict({
    'train': combine_data(raw_dataset, 'train'),
    'test': combine_data(raw_dataset, 'test')
})
