...
from accelerate.utils import is_deepspeed_available, gather_object
...
class SPINTrainer(Trainer):
    ...
    def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        policy_attention_mask = (policy_output != self.tokenizer.pad_token_id).long()
        # policy_output_decoded = self.tokenizer.batch_decode(policy_output, skip_special_tokens=True)

        # reference_output = pad_to_length(reference_output, self.max_length, self.tokenizer.pad_token_id)
        # reference_output_decoded = self.tokenizer.batch_decode(reference_output, skip_special_tokens=True)

        return policy_output, policy_attention_mask
    
    
    def get_reward(self, tokenizer, model, reward_model, reward_tokenizer, policy_output):
        ...
        for i in range(len(policy_output_decoded)):
            question, answer = policy_output_decoded[i].split("\n<|assistant|>\n")
            question = question.replace("<|user|>\n","")
            questions.append(question)
            answers.append(answer)

        inputs = reward_tokenizer(questions, answers, return_tensors = 'pt', padding = True)
        device = reward_model.device
        inputs = {k: v.to(device) for k,v in inputs.items()}
        with torch.no_grad():
            scores = reward_model(**inputs).logits
        return scores
    ...
    def spin_loss(
        ...
        opponent_logps: torch.FloatTensor,
        ...
        kl_divergence = policy_logps - opponent_logps
        logits = reward - self.beta * kl_divergence


        if self.loss_type == "sigmoid":
            losses = -F.logsigmoid(logits)
        elif self.loss_type == "hinge":
            losses = torch.relu(logits)
        else:
            raise ValueError(f"Unknown loss type: {self.loss_type}. Should be one of ['sigmoid', 'hinge']")

        print(f"losses: {losses}")
        print(f"policy_logps: {policy_logps}")
        print(f"opponent_logps: {opponent_logps}")
    ...
    def concatenated_forward(
        self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], policy_output, policy_attention_mask
        ...
        for i in range(len(batch['prompt_input_ids'])):
            prompt_length = len(batch['prompt_input_ids'][i])
            response = policy_output[i, prompt_length:]
            logits = all_logits[i, prompt_length:]
            labels.append(response)
            response_logits.append(logits)
        labels_tensor = torch.stack(labels)
        response_logits_tesnor = torch.stack(response_logits)

        all_logps = self._get_batch_logps(
            response_logits_tesnor,
            labels_tensor,
    ...
    def get_batch_metrics(
        ...
        metrics = {}
        policy_output, policy_attention_mask = self.get_batch_samples(self.model, prompts)
        policy_attention_mask = gather_object(policy_attention_mask)
   
        reward = self.get_reward(self.tokenizer, self.model, self.reward_model, self.reward_tokenizer, policy_output)

        (
            policy_logps,
            policy_logits,
        ) = self.concatenated_forward(model, batch, policy_output, policy_attention_mask)
        with torch.no_grad():
            if self.ref_model is None:
                with self.null_ref_context():
                    (
                        opponent_logps,
                        opponent_logits
                    ) = self.concatenated_forward(self.model, batch, policy_output, policy_attention_mask)
            else:
                (
                    opponent_logps,
                    opponent_logits
                ) = self.concatenated_forward(self.ref_model, batch, policy_output, policy_attention_mask)
        

        losses, rewards = self.spin_loss(
            reward,
            policy_logps,
            opponent_logps,
        )
        ...
        metrics[f"{prefix}rewards"] = rewards.cpu().mean()
        metrics[f"{prefix}logps/policy"] = policy_logps.detach().cpu().mean()
        metrics[f"{prefix}logps/opponent"] = opponent_logps.detach().cpu().mean()
        metrics[f"{prefix}logits/policy"] = policy_logits.detach().cpu().mean()
        metrics[f"{prefix}logits/opponent"] = opponent_logits.detach().cpu().mean()
